...
const triggerNodes: NodeTypeOption[] = [
  ...
  },
  {
    ...
    description:
      "Runs when a WhatsApp message arrives via the Business Cloud API",
    icon: "/whatsapp.svg",
  },
];
  ...
  const connectedAppNodes = useMemo(() => {
    ...
    const activeSlugs = new Set(
      connectedAccounts.items
        ...
        .map((a) =>
          (
            a.toolkit?.slug ?? (a as { toolkitSlug?: string }).toolkitSlug
          )?.toLowerCase(),
        )
          ...
          {connectedAppNodes.length === 0 && (
            <div className="p-4">
              ...
              >
                ...
                <span>
                  Connect Notion, Slack, Gmail & more in{" "}
                  <strong>Integrations</strong> to add them here
                </span>
              </Link>
            </div>
          )}
